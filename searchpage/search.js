...
function searchQuery(query, isLogedIn, username)
{

	var xhr = new XMLHttpRequest();
	xhr.open('POST', '../controllers/searchpage.php/', true);
	...
	xhr.onreadystatechange = function()
	{
		if(xhr.readyState == 4 && xhr.status == 200)
		{
			...
			console.log(data);
			showPosts(data, isLogedIn, username);
		}
	}
...
function showResults(searchData, isLogedIn, currentUser)
{
	...
	for (var i = 0; i < searchData.length; i++) {
		...
		for(let j = 0; j < searchData[i].ques_options.length;j++){
			...
			radio.setAttribute("name",searchData[i].id);
			radio.setAttribute("value",searchData[i].ques_options[j].choice);
		...
		thSpan.innerText = `${searchData[i].likes.length} likes`;
	...
	function userLikedIndex(i){
		let isLiked = null;
		for(let j = 0;j < searchData[i].likes.length;j++){
			if(searchData[i].likes[j] == currentUser){
				isLiked = j;
			}
		}
		return isLiked;
	...
	function wrapper(searchData,isLogedIn,currentUser){
		for(let i = 0;i<quesBox.length;i++){
			...
			if(searchData[i].likes.indexOf(currentUser) > -1){
			...
			like_btn.addEventListener('click', () => {
				if(isLogedIn){
					...
					if(searchData[i].likes.indexOf(currentUser) > -1){
						searchData[i].likes.splice(searchData[i].likes.indexOf(currentUser),1);
						likes.innerText = `${searchData[i].likes.length} likes`;
						likes.style.color = `grey`;
						like_btn_symbol.style.color = `grey`;
						updateData(searchData[i]);
					}else{
						searchData[i].likes.push(currentUser);
						likes.innerText = `${searchData[i].likes.length} likes`;
						likes.style.color = `var(--sred)`;
						like_btn_symbol.style.color = `var(--sred)`;
						updateData(searchData[i]);
			...
			if(searchData[i].votedby.indexOf(currentUser) > -1){
			...
			quesOptions.addEventListener("submit", (e) => {
				...
				if(isLogedIn){
					...
					if(searchData[i].votedby.indexOf(currentUser) > -1){
						...
						searchData[i].ques_options[optionIndex].votes += 1;
						searchData[i].votedby.push(currentUser);
	...
	wrapper(searchData, isLogedIn, currentUser);
	...
	let quesOptions = [];
	let optionsValue = [];
	let ranCol = randomColor(searchData[i].ques_options.length);
	for(let j =0; j < searchData[i].ques_options.length;j++){
		
		quesOptions.push(searchData[i].ques_options[j].choice);
		optionsValue.push(searchData[i].ques_options[j].votes);
	}	
